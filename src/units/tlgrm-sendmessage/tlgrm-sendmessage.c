...
nxs_chat_srv_err_t nxs_chat_srv_u_tlgrm_sendmessage_push(nxs_chat_srv_u_tlgrm_sendmessage_t *u_ctx)
{
	...
	if(nxs_chat_srv_d_tlgrm_request(NXS_CHAT_SRV_TLGRM_REQUEST_TYPE_SEND_MESSAGE, &message, NULL, &u_ctx->response_buf) !=
	   NXS_CHAT_SRV_E_OK) {

		nxs_log_write_error(&process, "[%s]: can't send tlgrm message (message: \"%r\")", nxs_proc_get_name(&process), &message);
...
nxs_chat_srv_err_t nxs_chat_srv_u_tlgrm_sendmessage_inline_keybutton_callback_add(nxs_chat_srv_u_tlgrm_sendmessage_t *      u_ctx,
                                                                                  size_t                                    pos_y,
                                                                                  size_t                                    pos_x,
