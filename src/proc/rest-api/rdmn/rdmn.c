...
#include <proc/rest-api/rdmn/rdmn.h>
#include <proc/rest-api/rdmn/ctx/rdmn-ctx.h>
#include <proc/rest-api/rdmn/rdmn-subproc.h>
...
void nxs_chat_srv_p_rest_api_rdmn_handler_post(nxs_rest_api_ctx_t *rest_api_ctx, nxs_rest_api_request_t *req, void *custom_ctx)
{
	...
	if(nxs_string_cmp(&nxs_chat_srv_cfg.rdmn.auth_token, 0, auth_token, 0) == NXS_NO) {
	...
	nxs_log_write_debug(&process,
	                    "[%s]: received message from redmine (auth token: \"%r\" (correct), post body: \"%R\")",
	                    nxs_proc_get_name(&process),
	                    auth_token,
	                    bdy_in,
	                    0);

	if(nxs_chat_srv_u_ra_queue_add(ra_queue_ctx, NXS_CHAT_SRV_M_RA_QUEUE_TYPE_RDMN_UPDATE, (nxs_string_t *)bdy_in) !=
	   NXS_CHAT_SRV_E_OK) {
